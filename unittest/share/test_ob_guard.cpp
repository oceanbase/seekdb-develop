...
// Test the behavior of the copy constructor
...
// Test the behavior of the assignment operator
...
// Test the usage behavior of pointers
...
TEST_F(TestObGuard, ob_shared_guard_from_uique_guard) {
  ...
  delete ptr2;
  ASSERT_EQ(ptr, shared_ptr.get_ptr());// strong exception safety guarantee
...
TEST_F(TestObGuard, ob_shared_guard_make_and_alloc) {
  ...
  ASSERT_EQ(&a, &shared_ptr->get_b());// Strong exception safety guarantee
...
TEST_F(TestObGuard, ob_weak_guard_copy_equal) {
  ObSharedGuard<TestObj> shared_ptr;
  ASSERT_EQ(OB_SUCCESS, ob_make_shared<TestObj>(shared_ptr));
  ObWeakGuard<TestObj> weak_gaurd = shared_ptr;// implicit construction
  ObWeakGuard<TestObj> weak_gaurd2;
  weak_gaurd2 = shared_ptr;// implicit construction and assignment
  weak_gaurd2 = weak_gaurd;// Same type assignment, no implicit construction
