...
void ObTxCtxTableTest::dump_ctx_with_merged_undo_action()
{
  ...
  // Initialize the parameters required for test cases
  ...
  // Execute a dump of the transaction context table to ensure that the multi-source transactions generated by the create table statements can be dumped, retain ctx can be released
  SLEEP_BEFORE_DUMP_TX_CTX = false;
  ...
  sleep(5);
  // Confirm that there is and only one transaction in the current context
  ...
  // Get the dump point of the current context table to confirm the success of the next dump
  ...
  // Generate an undo action, while inserting some new data
  ...
  WRITE_SQL_BY_CONN(tt1_conn, "insert into test_tx_ctx_t select a, b from test_tx_ctx_t;");
  // Dump ctx table, set SLEEP_BEFORE_DUMP_TX_CTX flag
  ...
  // Wait for a while to ensure that the context table dump has obtained tx data with undo node
  while (!HAS_GOT_TX_CTX) {
    fprintf(stdout, "waitting for scheduling tx ctx table merge dag ...\n");
    sleep(2);
  }
  // Again execute rollback, expect to merge the undo action generated by the first rollback
  ...
  // Confirm context table dump successful
