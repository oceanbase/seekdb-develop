...
public:
                     ...
                     const RowSelector row_sel = RowSelector{}) override
  {
    ...
    if (param_exprs.count() == 1
      ...
      ret = add_batch_rows<ObVectorBase>(agg_ctx, skip, bound, *param_expr, agg_col_id, agg_cell, row_sel);
      if (OB_FAIL(ret)) {
        SQL_LOG(WARN, "add batch rows failed", K(ret), K(param_exprs.at(0)->datum_meta_));
      }
...
#define INNER_ADD(vec_tc)                                                                          \
  case (vec_tc): {                                                                                 \
    ...
    if (OB_ISNULL(param_expr)) { // count(*)
      ...
      ret = inner_add_for_multi_groups<ObVectorBase>(agg_ctx, agg_rows, row_sel, batch_size, agg_col_id, param_expr);
    }
    return ret;
#undef INNER_ADD
  }
                                  ...
                                  const bool init_vector = true) override
  {
    ...
    if (loop_cnt <= 0) {
      ...
      if (init_vector && OB_FAIL(agg_expr.init_vector_for_write(
        ...
        ret = collect_group_results<ObVectorBase>(agg_ctx, agg_col_id, cur_group_id, output_start_idx, loop_cnt, skip);
        if (OB_FAIL(ret)) {
          SQL_LOG(WARN, "collect batch group results failed", K(ret));
        } else {
                                  ...
                                  const int32_t row_start_idx = 0, const bool need_init_vector = true) override
  {
    ...
    if (OB_LIKELY(batch_size > 0)) {
      if (need_init_vector && OB_FAIL(agg_expr.init_vector_for_write(
        ...
        ret = collect_group_results<ObVectorBase>(agg_ctx, agg_col_id, output_start_idx, batch_size,
                                                  rows, row_meta, row_start_idx, need_init_vector);
