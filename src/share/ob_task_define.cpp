...
public:
  ...
  ObSyslogPerErrLimiter() : inited_(false), refresh_task_(*this)
  {
  ...
  {
    if (inited_) {
      TG_STOP(lib::TGDefIDs::SyslogPerErrLimiter);
    }
  }
  void wait()
  {
    if (inited_) {
      TG_WAIT(lib::TGDefIDs::SyslogPerErrLimiter);
    }
  }
  void destroy()
  {
    if (inited_) {
      TG_DESTROY(lib::TGDefIDs::SyslogPerErrLimiter);
      inited_ = false;
    }
  }
...
private:
  ...
  class RefreshTask : public common::ObTimerTask
  {
  public:
    RefreshTask(ObSyslogPerErrLimiter &limiter) : limiter_(limiter) {}
...
int ObSyslogPerErrLimiter::init()
{
  ...
  if (OB_FAIL(TG_START(lib::TGDefIDs::SyslogPerErrLimiter))) {
    LOG_WARN("fail to start SyslogPerErrLimiter timer", K(ret));
  } else if (OB_FAIL(TG_SCHEDULE(lib::TGDefIDs::SyslogPerErrLimiter, refresh_task_,
                                 REFRESH_INTERVAL_US, true/*repeat*/, false/*immediate*/))) {
    LOG_WARN("fail to schedule SyslogPerErrLimiter timer", K(ret));
  }
