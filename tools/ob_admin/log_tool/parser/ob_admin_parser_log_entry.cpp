...
int ObAdminParserLogEntry::parse_data_dict_log_()
{
  int ret = OB_SUCCESS;
  ret = OB_NOT_SUPPORTED;
  LOG_ERROR("observer lite does not support data dict log", KR(ret));
  return ret;
}
  ...
  if (LogFormatFlag::TX_FORMAT == str_arg_.flag_
    ...
    switch (header.get_log_type()) {
      ...
      case oceanbase::logservice::ObLogBaseType::GC_LS_LOG_BASE_TYPE: {
        fprintf(stdout, "  Unsupported Log Type: GC_LS_LOG_BASE_TYPE\n");
        LOG_WARN("don't support GC_LS_LOG_BASE_TYPE log type");
