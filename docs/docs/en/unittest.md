...
[OceanBase SeekDB](https://github.com/oceanbase/oceanbase-seekdb) has two unittest directories.

- `unittest` . These are the main unittest cases, and it tests the code in the `src` directory.

- `deps/oblib/unittest`. Cases for oblib.

First, you should build `unittest`. You should enter the `unittest` directory in the build directory and build explicitly. When you build the seekdb project, it does't build the unittest in default. For example:
...
As a C++ project, [OceanBase SeekDB](https://github.com/oceanbase/oceanbase-seekdb) uses [google test](https://github.com/google/googletest) as the unittest framework.

SeekDB use `test_xxx.cpp` as the unittest file name. You can create a `test_xxx.cpp` file and add the file name into the specific `CMakeLists.txt` file.
