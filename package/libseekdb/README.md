...
CI macOS builds set **CMAKE_OSX_DEPLOYMENT_TARGET=11.0**, so the prebuilt `libseekdb.dylib` runs on **macOS 11 (Big Sur) and later** (12, 13, 14). Building on macOS 14 without this would only support macOS 14+; setting it to 11.0 allows use on most current and recent macOS versions.
...
- **OS and architecture**: The zip name reflects the build OS and CPU: `darwin-x64`, `darwin-arm64`, `linux-x64`, `linux-arm64` (x64 = x86_64). Use the matching zip for the target environment. On Linux, the prebuilt .so requires glibc ≥ 2.17 (see [Linux glibc compatibility](#linux-glibc-compatibility)), including CentOS 7; on macOS, the prebuilt dylib is built with **minimum deployment target 11.0**, so it runs on **macOS 11 (Big Sur) and later** (including 12–14).
