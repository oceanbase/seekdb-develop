  ...
  for (int l = 0; l < MAX_TEST_LOOP; l++) {
    for (int i = 0; i < MAX_TEST_COUNT; i++) {
      for (int j = 0; j < MAX_TEST_COUNT; j++) {
        ...
        length = ob_gcvt_strict(darr[i] * (1 == l%2 ? 1 : -1), OB_GCVT_ARG_DOUBLE, sizeof(buf) - 1, buf, NULL, TRUE, FALSE);
        ...
        MEMSET(buf, 0, MAX_DOUBLE_STRICT_PRINT_SIZE2);
        length = ob_gcvt_strict(darr[j] * (1 == l%2 ? 1 : -1), OB_GCVT_ARG_DOUBLE, sizeof(buf) - 1, buf, NULL, TRUE, FALSE);
...
TEST(ObNumber, round_even_number)
{
  ...
  for (int l = 0; l < MAX_TEST_LOOP; l++) {
    for (int i = 0; i < MAX_TEST_COUNT; i++) {
        ...
        length = ob_gcvt_strict(tmp_darr, OB_GCVT_ARG_DOUBLE, sizeof(buf) - 1, buf, NULL, TRUE, FALSE);
        EXPECT_EQ(OB_SUCCESS, num1.from(buf, length, allocator));
      ...
      for (int64_t m = 0; m < j; ++m) {
        ...
        *const_cast<char *>(ffi.ptr() + m) = '.';
        ...
        for (int d = 0; d < MAX_TEST2_COUNT; d++) {
          ...
          length = ob_gcvt_strict(test_rd, OB_GCVT_ARG_DOUBLE, sizeof(buf) - 1, buf, NULL, TRUE, FALSE);
          ObNumber value_old,value_new,value_new2;
