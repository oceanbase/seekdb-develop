...
class ObLinkQueueImpl
{
public:
  typedef QLink Link;
  ObLinkQueueImpl() : queue_(), push_(0), pop_(0)
  {}
  ~ObLinkQueueImpl() {}
  int push(Link *p)
  {
  ...
  int merge(ObLinkQueueImpl &other)
  {
    int ret = OB_SUCCESS;
    while(other.size() > 0) {
      ObLink *p = NULL;
...
private:
  DISALLOW_COPY_AND_ASSIGN(ObLinkQueueImpl);
};

using ObLinkQueue = ObLinkQueueImpl<>;
using ObLinkQueue16 = ObLinkQueueImpl<16>;
